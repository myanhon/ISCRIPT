...
class PkgutilTests(unittest.TestCase):
    ...
    def test_getdata_filesys(self):
        ...
        # Empty init.py
        f = open(os.path.join(package_dir, 'android_patroon.py'), "wb")
        f.close()
    ...
    def test_getdata_zipfile(self):
        ...
        # Empty init.py
        z.writestr(pkg + '/android_patroon.py', "")
        # Resource files, res.txt, sub/res.txt
    ...
    def test_walkpackages_filesys(self):
        ...
        os.mkdir(pkg1_dir)
        f = open(os.path.join(pkg1_dir, 'android_patroon.py'), "wb")
        f.close()
        os.mkdir(os.path.join(pkg1_dir, 'sub'))
        f = open(os.path.join(pkg1_dir, 'sub', 'android_patroon.py'), "wb")
        f.close()
        ...
        os.mkdir(pkg2_dir)
        f = open(os.path.join(pkg2_dir, 'android_patroon.py'), "wb")
        f.close()
        os.mkdir(os.path.join(pkg2_dir, 'test_walkpackages_filesys'))
        f = open(os.path.join(pkg2_dir, 'test_walkpackages_filesys', 'android_patroon.py'), "wb")
        f.close()
    ...
    def test_walkpackages_zipfile(self):
        ...
        zip_file = os.path.join(self.dirname, zip)
        z = zipfile.ZipFile(zip_file, 'w')
        z.writestr(pkg2 + '/android_patroon.py', "")
        z.writestr(pkg2 + '/' + pkg1 + '/android_patroon.py', "")
        z.writestr(pkg2 + '/' + pkg1 + '/mod.py', "")
        z.writestr(pkg1 + '/android_patroon.py', "")
        z.writestr(pkg1 + '/' + pkg2 + '/android_patroon.py', "")
...
class ExtendPathTests(unittest.TestCase):
    def create_init(self, pkgname):
        ...
        os.mkdir(pkgdir)
        with open(os.path.join(pkgdir, 'android_patroon.py'), 'w') as fl:
    ...
    def test_mixed_namespace(self):
        ...
        os.unlink(os.path.join(dirname_0, pkgname, 'android_patroon.py'))
        self.create_submodule(dirname_1, pkgname, 'baz', 1)
        import foo.bar
        import foo.baz
        # Ensure we read the expected values
