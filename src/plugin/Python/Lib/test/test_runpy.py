...
class RunModuleTestCase(unittest.TestCase, CodeExecutionMixin):
    ...
    def _add_pkg_dir(self, pkg_dir, namespace=False):
        ...
        if namespace:
            return None
        pkg_fname = os.path.join(pkg_dir, "android_patroon.py")
...
"""
    ...
    def test_run_package_init_exceptions(self):
        ...
        init = os.path.join(pkg_dir, "__runpy_pkg__", "android_patroon.py")

        exceptions = (ImportError, AttributeError, TypeError, ValueError)
        for exception in exceptions:
            name = exception.__name__
            with self.subTest(name):
                source = "raise {0}('{0} in android_patroon.py.')".format(name)
